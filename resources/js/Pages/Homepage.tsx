...
type User = {
    ...
    name: string;
    kaderisasi: string;
    img: string; // Tambahkan properti `img`
    rayon: {
        rayon: string;
    };
};
...
type HomepageProps = {
    ...
    totalPKN: string;
    users: {
        data: User[]; // `users` memiliki properti `data` yang berupa array
    };
};

const Homepage: React.FC<HomepageProps> = ({ totalMapaba, totalPKD, totalPKL, totalPKN, users }) => {
    console.log(users);
